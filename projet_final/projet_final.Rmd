...
top_indices <- order(variances, decreasing = TRUE)[1:min(30, length(variances))]
...
Une fois que le modèle a bien été entraîné sur les 5 blocs, nous pouvons examiner les résultats moyens
...
metrics <- data.frame(
  Mesure = c("Accuracy", "Kappa (CV 5-fold)", "Sensibilité (Rappel)"),
  Valeur = c(
...
cat("Nombre de features TF-IDF initiales :", ncol(X), "\n")
...
Enfin, nous avons utilisé un classifieur Bayésien Naïf sur les axes de l'AFD, avec validation croisée en 5 blocs pour évaluer la performance du modèle.

**Résultats obtenus :**

Notre modèle a atteint les performances suivantes :
- **Textes humains** : 77.32% correctement identifiés
- **Textes générés par IA** : 79.16% correctement identifiés
...
Combiner AFD et Bayésien naïf est une approche efficace pour détecter les textes humains des textes IA. 
Nous avons obtenu de bons résultats avec une performance d'environ 77-79 %, ce qui signifie que notre modèle est capable de 
détecter correctement la majorité des textes, même s'il existe encore une part d'erreur (environ 20-23%).

On peut donc en conclure qu'il est possible de différencier les textes générés par une IA de ceux écrits par des humains avec une bonne précision, 
mais il reste encore une marge d'amélioration pour atteindre une performance parfaite.
</DIV>
